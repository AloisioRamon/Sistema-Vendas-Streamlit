...
def inserir_clientes(nome, sexo, email):
    conexao = conectar()
    cursor = conexao.cursor()
    cursor.execute("""
...
def atualizar_clientes(id, nome, sexo, email):
    ...
    cursor = conexao.cursor()
    cursor.execute("""
        UPDATE clientes SET nome=?, sexo=?, email=? WHERE id=?
    """, (nome, sexo, email, id))
    conexao.commit()
    conexao.close()
...
if escolha == "Cadastrar":
    ...
    if submit:
        inserir_clientes(nome, sexo, email)
    ...
    if not clientes:
        ...
        for c in clientes:
            with st.expander(f"{c[1]} - {c[2]}"):
                ...
                novo_sexo = st.selectbox("Sexo", ["Masculino", "Feminino", "Outro"], 
